# MANIFEST: {"name": "pollution_rose", "description": "Pollution rose: wind rose variant showing pollutant concentration by direction and speed. Requires ws, wd, and at least one pollutant series.", "input_type": "wind_series", "output_type": "image", "timeout": 60}
...
outfile <- file.path(artifacts_dir, paste0("pollution_rose_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png"))

grDevices::png(outfile, width = 900, height = 900, res = 120)
openair::pollutionRose(df, pollutant = pollutant, main = plot_title)
invisible(grDevices::dev.off())

cat(jsonlite::toJSON(list(
  artifact = normalizePath(outfile, winslash = "/", mustWork = FALSE),
  type = "png",
  summary = paste0("Pollution rose for ", summary_labs[1]),
  tool = "pollution_rose"
), auto_unbox = TRUE))
