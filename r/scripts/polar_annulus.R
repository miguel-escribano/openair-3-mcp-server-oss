# MANIFEST: {"name": "polar_annulus", "description": "Bivariate polar annulus plot. Useful for exploring the relationship between a pollutant, wind, and a third variable (e.g. season, time of day).", "input_type": "wind_series", "output_type": "image", "timeout": 90}
...
outfile <- file.path(artifacts_dir, paste0("polar_annulus_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png"))

grDevices::png(outfile, width = 900, height = 900, res = 120)
openair::polarAnnulus(df, pollutant = pollutant, main = plot_title)
invisible(grDevices::dev.off())

cat(jsonlite::toJSON(list(
  artifact = normalizePath(outfile, winslash = "/", mustWork = FALSE),
  type = "png",
  summary = paste0("Polar annulus for ", summary_labs[1]),
  tool = "polar_annulus"
), auto_unbox = TRUE))
