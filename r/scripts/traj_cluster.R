# MANIFEST: {"name": "traj_cluster", "description": "K-means clustering of back trajectories. Groups air mass origins into distinct types.", "input_type": "traj_series", "output_type": "image", "timeout": 300}
...
outfile <- file.path(artifacts_dir, paste0("traj_cluster_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png"))

grDevices::png(outfile, width = 1400, height = 700, res = 120)
openair::trajCluster(df, n.clusters = 5)
invisible(grDevices::dev.off())

cat(jsonlite::toJSON(list(
  artifact = normalizePath(outfile, winslash = "/", mustWork = FALSE),
  type = "png",
  summary = "Trajectory cluster analysis (5 clusters)",
  tool = "traj_cluster"
), auto_unbox = TRUE))
