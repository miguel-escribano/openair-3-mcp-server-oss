# MANIFEST: {"name": "traj_level", "description": "Trajectory level plot: gridded concentration heat map based on trajectory frequency. Reveals source regions.", "input_type": "traj_series", "output_type": "image", "timeout": 180}
...
outfile <- file.path(artifacts_dir, paste0("traj_level_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png"))

grDevices::png(outfile, width = 1200, height = 900, res = 120)
openair::trajLevel(df)
invisible(grDevices::dev.off())

cat(jsonlite::toJSON(list(
  artifact = normalizePath(outfile, winslash = "/", mustWork = FALSE),
  type = "png",
  summary = "Trajectory level (source contribution) plot",
  tool = "traj_level"
), auto_unbox = TRUE))
